...
No entanto a demanda por iniciativas capazes de lidar com estes dilemas tem aumentado. O padrão IEEE 802.15.4 \cite{IEEE2003} foi especificamente concebido para utilização em sistemas de comunicação de baixa potência, baixo custo e que operem com baixas taxas de transferência. Este padrão destina-se à aplicações que requeiram links de comunicação de curto alcance e alta eficiência energética, nele são especificadas implementações para as duas camadas inferiores da pilha de protocolos: a camada física (PHY) e a camada de enlace, ou controle de acesso ao meio (MAC), conforme definidas pelo modelo OSI (apresentado na \autoref{tab_osi}). A arquitetura das camadas superiores é deferida aos diversos protocolos que implementam o IEEE 802.15.4, como 6LoWPAN \cite{Montenegro2007}, ZigBee \cite{ZigBee2005}, WirelessHART \cite{Kim2008}.

O padrão IEEE 802.15.4 define dois tipos de nodos: dispositivos com funcionalidade completa (\textit{full-functioning device}, ou FFD) e dispositivos com funcionalidade reduzida (\textit{reduced-function device}, ou RFD). RFDs são dispositivos dotados de menor capacidade de processamento e memória, localizados nas extremidades das redes e cuja funcionalidade se resume a estabelecer comunicação com algum FFD. Já os FFDs são dispositivos com maior capacidade embarcada de processamento e capazes de implementar completamente o padrão, são responsáveis por coordenar as atividades de comunicação e roteamento. Também é definido no IEEE 802.15.4 o suporte a duas topologias de rede: estrela e \textit{peer-to-peer}. \cite{Rawat2014}. Ao utilizar a topologia estrela todos os nodos devem se comunicar com dispositivo controlador central, do tipo FFD. Já através da topologia \textit{peer-to-peer} é possível estabelecer links de comunicação entre dois nodos quaisquer, o que permite que se produzam rede mais complexas, como por exemplo redes \textit{mesh}, onde diversos nodos dispostos de forma potencialmente irregular colaboram para estabelecer rotas de comunicação entre si, mesmo que não sejam capazes de se comunicar diretamente.

A versão original do padrão, IEEE 802.15.4-2003 \cite{IEEE2003}, previa a operação somente nas faixas de 868 MHz e 915 MHz (através da modulação BPSK com taxa máxima de transferência de 40 kbps), e na faixa de 2.4 GHz (através da modulação O-QPSK com taxa máxima de transferência de 250kbps). No entanto ao longo do tempo diversas revisões e emendas foram adicionadas.

Em sua primeira revisão, o IEEE 802.15.4-2006 \cite{IEEE2006}, foi incorporado o suporte à duas definições adicionais para a camada física, permitindo a utilização de taxas de transferência mais elevadas nas bandas de frequência de 868 MHz e 915 MHz, através da utilização de modulações O-QPSK e ASK, capazes de atingir 100 kbps e 250 kpbs respectivamente. Ainda nesta revisão foram adicionadas definições para mecanismos de segurança e outros aprimoramentos na camada MAC, como o suporte para uma base de tempo compartilhada, agendamento de \textit{beacons} e a sincronização de mensagens de broadcast.

Apenas um ano após a aprovação desta revisão o padrão recebeu uma emenda, originando o IEEE 802.15.4a \cite{IEEE2007}, através do qual foram incorporadas mais duas especificações para PHYs opcionais: o \textit{Chirp Spread Spectrum} (CSS), capaz de elevar a taxa de transferência na faixa de 2450 MHz para 1000 kbps, e o Ultra-wide Band (UWB) destinado à operação nas faixs de 3 GHz, 5 GHz e 6 GHz a 10 GHz, capaz de fornecer taxas de transferência de até 27.4 Mbps.

As definições do PHY \textit{Ultra-wide Band} se baseiam em técnicas de transmissão que utilizam pulsos de rádio extremamente curtos, emitidos em sequências periódicas  \cite{Porcino2003}. O sinal de transmissão UWB possui ocupação espectral instantânea superior a 500 MHz e largura fracional de banda superior a 20\%, o que permite alta eficiência espectral e altas taxas de transmissão com baixo consumo energético, além de proporcionar alta tolerância à ambientes onde há propagação multi-caminho. No entanto esta tecnologia requer maior largura de banda do que a disponibilizada nas faixas de frequência ISM, sendo este o único PHY definido pelo IEEE 802.15.4 que não as utiliza. Adicionalmente, esta tecnologia não é indicada para transmissões de longa distância ou para utilização em ambientes perigosos devido à emissão de pulsos de alta energia \cite{Rawat2014}.


IEEE 802.15.4c-2009 Chinese 780 MHz PHY

IEEE 802.15.4d-2009 Japanese 950 MHz PHY

IEEE 802.15.4-2011 Atualização da redação??

IEEE 802.15.4e-2012 New MAC frames + CWPAN

IEEE 802.15.4f-2012 MSK PHY / LR PRF UWB PHY + MAC changes

IEEE 802.15.4g-2012 SUN PHYs (MR-FSK / MR-OFDM / MR-O-QPSK) <- Multiregional PHYs

IEEE 802.15.4n-2016 PHY 174-216MHz, 407-425 MHz, 608-630 MHz + MAC

IEEE 802.15.4q-2016 Ultra Low Power PHYs



 

Se comparado a outros padrões bem difundidos, o IEEE 802.15.4 permite taxas de transferência menores que as disponíveis no IEEE 802.11, mas maior alcance que do que seria possível utilizando-se o IEEE 802.15.1 \cite{IEEE2005}. Contudo o IEEE 802.15.4 se destaca por possibilitar menor consumo energético, quando comparado aos demais padrões de série IEEE 802.$X$.
