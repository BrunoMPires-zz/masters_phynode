...
\begin{itemize}
	\item \textit{Sistema de alimentação.} Responsável pelo fornecimento de energia aos demais componentes, devendo regular a voltagem e realizar outras adequações que se façam necessárias nas linhas de alimentação do nodo. Pode ainda realizar a captura e o armazenamento da energia proveniente de fontes secundárias (externas), quando disponíveis. É essencialmente composto por uma ou mais baterias primárias e um conversor DC-DC, capaz de manter, de forma eficiente, os níveis de voltagem adequados nas linhas de alimentação;
	...
	\item \textit{Sistema de processamento.} É responsável por coordenar as tarefas delegadas ao nodo como, por exemplo, a obtenção de dados amostrais ou o roteamento de pacotes de dados encaminhados por outros nodos. Este sistema é composto essencialmente por um microcontrolador com memória integrada, podendo opcionalmente contar com módulos de memória adicionais, a fim de permitir o armazenamento de maiores quantidades de dados (o que pode vir a ser necessário de acordo com a topologia da rede ou caso não haja disponibilidade de conexões com o exterior);
...
\begin{itemize}
	...
	\item \textit{Interferência.} É causada pela presença de múltiplos sinais sendo transmitidos concorrentemente, num mesmo ambiente, através de faixas de frequência similares. É possível inclusive que estes sinais originem-se em diferentes nodos de uma mesma rede de sensores sem fios. Segundo \citeonline{Baccour2012}, a área sobre a qual transmissões de um nodo podem causar interferência destrutiva supera a área na qual este nodo consegue manter links de comunicação confiáveis, logo podem-se estabelecer relações de interferência mesmo entre nodos que não sejam vizinhos diretos. Fontes de interferência podem ainda incluir outros equipamentos que compartilhem as mesmas faixas de frequência para comunicação sem fios ou mesmo ruído eletromagnético gerado involuntariamente por certos equipamentos elétricos.
\end{itemize}
...
\section{Sistema de processamento}
...
\section{Sistemas de alimentação}
...
O maior diferencial da plataforma PhyNode, no entanto, é seu sistema de comunicação, capaz de operar através de múltiplas interfaces de comunicação simultaneamente. A seguir serão apresentados os componentes utilizados neste sistema e serão discutidos os avanços obtidos e as capacidades dos mesmos.
...
A fim de validar experimentalmente as propostas realizadas nos capítulos \ref{phynode} e \ref{phymac} num ambiente real, foi estabelecida uma rede multi-salto composta por sete nodos. Estes nodos foram dispostos em um ambiente residencial a uma distância regular de cerca de quinze metros. Buscou-se dispor os nodos de forma que cada um dos saltos estivesse exposto a um ambiente ligeiramente diferente dos demais, incluindo espaços abertos, fechados, vegetação densa e por fim um ambiente de alta umidade. Vale ressaltar que não existiam nestes ambientes outras fontes notáveis de interferência além de dois roteadores WiFi operando na banda de 2.4 GHz.

É importante também notar que o desempenho dos links de comunicação pode ser afetado significativamente por uma série de parâmetros configuráveis, como a seleção do canal utilizado ou a potência de transmissão, além de fatores externos. Para realização dos experimentos em ambiente real o valor destes parâmetros foi escolhido manualmente, de forma a melhor permitir a visualização dos efeitos do ambiente sobre as diferentes interfaces disponíveis. Mais especificamente, o canal selecionado para cada uma das interfaces foi escolhido de forma a reduzir a intensidade do ruído de fundo (avaliado através de uma varredura realizada pela própria plataforma de testes), sendo o valor escolhido mantido constante para todos os nodos. A potência de transmissão foi reduzida, com o objetivo de permitir que houvesse um certa porcentagem de falhas na troca de pacotes entre os nodos, possibilitando melhor visualização do comportamento dos algoritmos implementados num ambiente real.

Adicionalmente, durante os testes não haviam mecanismos de roteamento implementados nos nodos, por este motivo foi definida uma topologia linear para a rede de testes. Esta estrutura foi previamente fixada através de parâmetros de configuração, que indicavam para cada nodo qual seria o endereço do vizinho com o qual ele deveria se comunicar. As extremidades desta estrutura não se comunicavam entre si, resultando em seis saltos que puderam ser monitorados para obtenção dos resultados experimentais. Durante os testes cada um dos nodos foi equipado com um cartão SD, através do qual arquivos de log puderam ser posteriormente recuperados, permitindo a análise do desempenho dos links e dos mecanismos de seleção de interface.

Como será descrito nas seções seguintes, os testes realizados buscaram validar em duas etapas as soluções propostas. Inicialmente foi avaliado o desempenho dos enlaces providos por protótipos baseados na arquitetura PhyNode. Já na segunda etapa foi avaliado o desempenho da solução composta por nodos baseados na arquitetura PhyNode operando em conjunto com os algoritmos de seleção de interfaces propostos em PhyMAC. 

\section{PhyNode}

Inicialmente foram realizados testes destinados a validar experimentalmente a arquitetura PhyNode. Estes testes tiveram como objetivo avaliar o desempenho dos enlaces de rádio providos pelo sistema de comunicação, baseado no conceito de diversidade de interfaces, que foi proposto por esta arquitetura. Estes testes consistiram na transmissão de um total de 5000 \textit{beacons}, separados por intervalos de tempo com duração fixa de 100ms. Cada uma das transmissões foi realizada, simultaneamente, através das três interfaces disponíveis. A escolha da quantidade de pacotes e intervalo de envio também se deu experimentalmente e teve como objetivo fornecer um espaço amostral suficientemente amplo para posterior análise dos dados, minimizando o tempo gasto com cada rodada de testes.
...
Os resultados encontram-se ilustrados na \autoref{fig_prr_no_opt} e na \autoref{fig_dpr_no_opt}. Os gráficos presentes na \autoref{fig_prr_no_opt} representam a PRR obtida a partir de cada uma das interfaces após a conclusão do teste. Há uma coluna adicional (\textit{PhyNode}), a qual representa os valores obtidos através de um contador de PRR implementado após a camada MAC, e expressa a PRR que seria efetivamente obtida por aplicações em execução nas camadas superiores da pilha de protocolos (uma vez que pacotes redundantes, recebidos por mais de uma interface, são descartados diretamente pela camada MAC). Pode-se observar que o comportamento dos links tende a variar de acordo com o ambiente onde são colocados, no entanto a PRR obtida por PhyNode é constantemente maior do que a obtida através de qualquer uma das interfaces isoladamente, comprovando que a diversidade de interfaces, da forma como foi implementada, tem o potencial para elevar a qualidade dos links de comunicação entre nodos de uma rede de sensores sem fios.

Adicionalmente, os gráficos presentes na \autoref{fig_dpr_no_opt} representam o nível de redundância decorrente do uso simultâneo de todas as interfaces. Cada uma das colunas representa o número de interfaces através das quais determinado pacote foi recebido. Por exemplo, um pacote totalmente perdido contribuiria para o aumento do valor percentual da coluna "0", já um pacote recebido simultaneamente pelas três interfaces iria contribuir para o aumento do percentual representado pela coluna "3". Durante a realização  destes testes, em condições ideais, os gráficos deveriam apresentar 100\% de recepção na coluna "3", porém como houveram perdas de pacotes a distribuição apresentada varia a cada salto.

\begin{figure}[b!]
	\centering
	\caption{Distribuição da recepção dos pacotes (Sem otimização)}
...
Os testes iniciais demonstraram que a arquitetura introduzida por PhyNode é capaz de prover enlaces robustos e confiáveis, através da utilização simultânea de múltiplas interfaces de comunicação. No entanto a eficiência energética com a qual a plataforma opera fica comprometida pela grande quantidade de pacotes redundantes que podem ser transmitidos nos casos em que mais de uma interface apresenta alta taxa de recepção (PRR). Os algoritmos de controle propostos em PhyMac são destinados à seleção dinâmica das interfaces de transmissão, com o objetivo de mitigar o \textit{overhead} energético causado por este cenário.

A fim de avaliar o desempenho destes algoritmos foram realizados novos testes baseados nos mesmos parâmetros utilizados anteriormente, porém utilizando PhyMAC para controlar os processos de transmissão e seleção de interfaces. Os resultados encontram-se ilustrados na \autoref{fig_prr_opt}, na \autoref{fig_dpr_opt} e na \autoref{fig_trng_no_opt}. Nas Figuras \ref{fig_prr_opt} e \ref{fig_dpr_opt} encontram-se gráficos equivalentes aos obtidos a partir dos testes realizados anteriormente, no entanto os resultados são bastante diferentes. Pode-se observar na \autoref{fig_prr_opt} que os valores de PRR obtidos por \textit{PhyNode} continuam consistentemente altos, porém o PRR médio apresentado pelas demais interfaces foi reduzido, indicando que houve redução da quantidade de pacotes redundantes transmitidos durante os testes.
...
Adicionalmente, é possível observar através dos gráficos presentes na \autoref{fig_trng_no_opt} os valores computados pelo LQE \textit{Triangle} ao longo da execução dos testes. As variações mais bruscas presentes nos valores iniciais se devem à movimentação física dos nodos, no início dos testes, para que pudessem ser reposicionados. Pode-se notar que os valores se tornam relativamente estáveis após o posicionamento final (por volta do milésimo pacote).
...
O presente trabalho teve como objetivo explorar alternativas para lidar com a instabilidade dos links de comunicação sem fios de baixa potência, frequentemente utilizados nas redes de sensores sem fios e outros sistemas de baixa potência derivados da mesma arquitetura. As propostas realizadas basearam-se na utilização de técnicas orientadas à diversidade de interfaces: um paradigma de implementação que visa explorar diferentes características físicas dos meios de transmissão, com o objetivo de permitir o estabelecimento de múltiplos links de comunicação independentes, potencialmente provendo conexões mais robustas e estáveis.

Inicialmente foi proposta uma arquitetura de hardware capaz de implementar a diversidade de interfaces através do espalhamento espectral fornecido por um conjunto de múltiplas interfaces de rádio, operando em bandas de frequência distintas. PhyNode, a arquitetura proposta, demonstrou-se experimentalmente capaz de prover links de comunicação de altíssima qualidade, no entanto a utilização simultânea de múltiplos transmissores provou-se pouco eficiente em termos de consumo energético, uma vez que grande parte das transmissões realizadas foi redundante.

Em seguida foi proposta uma extensão à camada MAC do padrão IEEE 802.15.4, com o objetivo de fornecer aos protocolos e aplicações operando em camadas superiores o suporte necessário para operação transparente na arquitetura PhyNode, abstraindo as funcionalidades relacionadas à diversidade de interfaces. A extensão proposta, PhyMAC, conta ainda com algoritmos de controle capazes de otimizar o consumo dos links de comunicação estabelecidos por PhyNode, visando reduzir o volume de transmissões redundantes realizadas.

Finalmente, os protótipos desenvolvidos foram submetidos a testes em ambientes reais, revelando que a utilização do conjunto de soluções propostas em PhyNode e PhyMAC pode consistir numa alternativa válida para obtenção de links de comunicação mais robustos, sem grande \textit{overhead} energético.

É necessário ainda considerar que a complexidade e o custo associados à produção de um nodo baseado na arquitetura PhyNode podem ser significativamente superiores àqueles associados com a produção de um nodo convencional, que não utilize técnicas de diversidade de interfaces. No entanto PhyNode, se utilizado em conjunto com PhyMAC, pode permitir a implantação de redes menos densas e mais robustas podendo, em alguns casos, ser uma solução mais vantajosa.

Adicionalmente, em teoria, o conjunto de \textit{hardware} e \textit{software} que compõe os nodos baseados em PhyNode e PhyMAC provê suporte à utilização de diversos protocolos de alto nível como ZigBee, 6LoWPAN e outros implementados a partir do padrão IEEE 802.15.4, abrindo caminho para a implementação de diversas outras camadas de software destinadas à interoperabilidade de dispositivos. Pode-se considerar a possibilidade de implementações capazes de integrar-se simultaneamente a redes baseadas em diferentes tecnologias e/ou protocolos, ou ainda de utilizar o conjunto PhyNode e PhyMAC para implementação de redes heterogêneas, formadas por diferentes tipos de nodos, cada qual operando através de uma tecnologia.
